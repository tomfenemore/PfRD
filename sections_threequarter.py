...
for i in range(buckle):
    ...
    rp = p.Set(referencePoints=(p.referencePoints.findAt((25,25,i)),), name='rp%s'%i,)
    good_nodes = mesh.MeshNodeArray((node11,node12,node21,node22))
    nds = p.Set(nodes=good_nodes, name='trial_set%s'%i)
    
    mdb.models[run].Coupling(name='trial%s'%i, surface=nds, controlPoint=rp, influenceRadius=100, couplingType=STRUCTURAL)
...
for i in range(buckle,1000):
    ...
    rp = p.Set(referencePoints=(p.referencePoints.findAt((6.28, 25, i)),), name='rp%s'%i, )
    good_nodes = mesh.MeshNodeArray((node11, node12, node21, node22))
    nds = p.Set(nodes=good_nodes, name='trial_set%s'%i)

    mdb.models[run].Coupling(name='trial%s' % i, surface=nds, controlPoint=rp, influenceRadius=100,
                             couplingType=STRUCTURAL)

    mdb.models[run].ConcentratedForce(name='L-%s' % i, createStepName='nlan', region=rp, cf2=2.8)
